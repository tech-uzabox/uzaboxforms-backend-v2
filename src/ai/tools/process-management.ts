import { tool } from 'ai';
import { ProcessForm } from 'db/client';
import { z } from 'zod';
import { PrismaService } from '../../db/prisma.service';
import { ProcessData, RolesData, StoredStepData } from './form-schemas';
...
export const createSaveProcessTool = (
      ...
      try {
        ...
        const stepData: StoredStepData = {
          ...
          applicantViewFormAfterCompletion:
            input.applicantViewFormAfterCompletion,
        ...
        if (existingSave) {
          const stepsData =
            (existingSave.stepsData as unknown as StoredStepData[]) || [];
      ...
      try {
        ...
        if (existingSave && existingSave.stepsData) {
          ...
          const updatedStepsData = stepsData.filter(
            (step: StoredStepData) =>
              step.processId !== input.stepId || step.formId !== input.formId,
          );
